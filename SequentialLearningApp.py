    ...
    def start_sequential_learning(self):
        ...
        for i in range(self.number_of_executions):
                ...
                while sum_.loc[self.SampIdx].max().values < targ_q_t.values:
                                #print(sum_.loc[self.SampIdx].max().values)
...
#        to_append=[np.mean(self.tries_list),np.std(self.tries_list),np.quantile(self.tries_list,0.90),np.quantile(self.tries_list,1),select_model.value, confirm_strategy(),self.sigma,self.distq,iterations.value,self.init_sample_size,len(self.dataframe.index),len(confirm_features()),len(confirm_target()),quantile_tar_slider.value,quantile_sample_slider.value]
    ...
    def perform_random_pick(self,acutal_iter):
        ...
        print('targ_q_t.values', targ_q_t.values)
        run=0
        df_rand=np.array([-10000])       
        self.tries_list_rand_pick[acutal_iter]=0
        while df_rand.item() < targ_q_t.item():
            df_rand = rand_sum[run]
            #print('while', df_rand.item() < targ_q_t.item())
            #print('type rand',type(df_rand.item()),df_rand.item())
            #print('type Targ',type(targ_q_t.item()),targ_q_t.item())
